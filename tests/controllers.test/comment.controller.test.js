...
describe('addComment', () => {
    test('Must recalculate averageGrade and return it in body with success message.', async () => {
        ...
        const response = await request(app).post('/api/comment').set('API-Key', process.env.HEADER_KEY).send(query);
        expect(response.body).toMatchObject({
    ...
    test('Must return error about APIKEY', async () => {
        const query = {
            cityName: citiesToAdd[0].cityName,
            commentText: 'Golubi i Khreshyatik',
            commentGrade: 7,
        };
        const response = await request(app).post('/api/comment').send(query);
        expect(response.body).toMatchObject({
...
describe('updateComment.', () => {
    test('Must return error about APIKEY', async () => {
        ...
        const response = await request(app).put(`/api/comment/${commentId}`).send(query);
        expect(response.body).toMatchObject({
            error: 'Api key is required for this function.',
        });
        expect(response.statusCode).not.toBe(200);
    ...
    describe('without new Grade', () => {
        test('must return newComment text and change it', async () => {
            ...
            const response = await request(app).put(`/api/comment/${commentId}`)
                .set('API-Key', process.env.HEADER_KEY).send(query);
            expect(response.body).toMatchObject({
        ...
        test('must return an ID error', async () => {
            ...
            const response = await request(app).put('/api/comment/124233453')
                .set('API-Key', process.env.HEADER_KEY).send(query);
            expect(response.body).toMatchObject({
